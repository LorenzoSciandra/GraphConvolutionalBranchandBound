...
int init_prob_edges(CCtsp_lp *lp){

    double graph_prob = 0.0;
    double edge_prob = 0.0;
    double count_edge = 0.0;
    int id_src, id_dest, edge_id, rval = 0;

    for (int j = 0; j < lp->result.ecount; j++){
        id_src = lp->result.elist[2*j];
        id_dest = lp->result.elist[2*j + 1];
        edge_id = CCtsp_find_edge(&lp->graph, id_src, id_dest);
        ...
        edge_prob = nn_edges_probs.edge_probs[lp->perm[id_src]][lp->perm[id_dest]];
        lp->graph.edges[edge_id].prob = edge_prob;
        graph_prob = graph_prob + edge_prob * lp->result.x[j];
        count_edge = count_edge + lp->result.x[j];

        //printf("Edge: (%d,%d) - (%d, %d), prob: %f, x: %f\n", id_src, id_dest, lp->perm[id_src], lp->perm[id_dest], lp->graph.edges[edge_id].prob, lp->result.x[j]); fflush(stdout); 

    }

    //printf("Graph prob: %f, Graph count: %f, Frac: %f\n", graph_prob, count_edge, graph_prob/count_edge); fflush(stdout);
        ...
        CCtsp_lpcuts *dominopool, int silent, CCrandstate *rstate)
{
    ...
    *lp = CC_SAFE_MALLOC (1, CCtsp_lp);
    ...
    if (USE_PVAR || USE_PNODE){
        ...
        rval = init_prob_edges(*lp);

        if (rval) {
            fprintf(stderr, "init_prob_edges failed\n");
            goto CLEANUP;
        }
    }
    // FINE MIE MODIFICHE
