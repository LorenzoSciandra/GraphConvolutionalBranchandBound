...
NNedges_probs nn_edges_probs;


int read_prob_file(int ncount){
    ...
    nn_edges_probs.pnode_ties_count = 0;
    nn_edges_probs.total_pvar_ties = 0;
    nn_edges_probs.pvar_ties_count = 0;
    nn_edges_probs.total_frac_ties = 0;
    nn_edges_probs.frac_ties_count = 0;
    nn_edges_probs.ecount = 0;
    double min_probs = 1.0;
    double max_probs = 0.0;
    nn_edges_probs.ncount = ncount;
    nn_edges_probs.edge_probs = malloc(sizeof(double *) * ncount); // Allocate for ncount rows

    for (int i = 0; i < ncount; i++) {
        nn_edges_probs.edge_probs[i] = malloc(sizeof(double) * (ncount)); // Allocate ncount elements for each row
        if (nn_edges_probs.edge_probs[i] == NULL) {
            fprintf(stderr, "Error allocating memory\n");
            return 1;
        }
    }

    if (nn_edges_probs.edge_probs == NULL) {
        fprintf(stderr, "Error allocating memory\n");
        return 1;
    }

    while (fscanf(fp, "%d %d %lf", &n1, &n2, &prob) != EOF){
        nn_edges_probs.edge_probs[n1][n2] = prob;
        nn_edges_probs.edge_probs[n2][n1] = prob;
        nn_edges_probs.ecount++;
    }


    double normalized_prob = 0.0;
    for (int i = 0; i < ncount; i++) {

        for(int j = 0; j < ncount; j++){
            if (nn_edges_probs.edge_probs[i][j] < min_probs) {
                min_probs = nn_edges_probs.edge_probs[i][j];
            }
            if (nn_edges_probs.edge_probs[i][j] > max_probs) {
                max_probs = nn_edges_probs.edge_probs[i][j];
            }

        }

        for (int j = 0; j < ncount; j++) {
            normalized_prob = (nn_edges_probs.edge_probs[i][j] - min_probs) / (max_probs - min_probs);
            nn_edges_probs.edge_probs[i][j] = normalized_prob;
        }
    }
...
int main (int ac, char **av)
{
    ...
    if (masterfname) {
        ...
        if (USE_PNODE || USE_PVAR) {
            ...
            filename = strrchr(datfname, '/');
            if (filename != NULL) {
                filename++;  // Move past the '/' character
            } else {
                fprintf(stderr, "Error in parsing the filename, not in '/examples/ directory\n");
                return 1;
            }
            ...
            if (dot != NULL) {
                ...
                strncpy(nn_edges_probs.filename, filename, len);
                nn_edges_probs.filename[len] = '\0';
            } else {
                fprintf(stderr, "Error in parsing the filename, no correct extension\n");
                return 1;
            }

            rval = read_prob_file(ncount);

            if (rval){
                fprintf(stderr, "Error reading probabilities file\n");
                return 1;
            }
        }
