...
static void
    ...
    insert_sblist (sbitem *list, double val, int name, double prob_edge),
    insert_sblist4 (sbitem *list, double val, int name, int name2, int name3,
                    int name4, double prob_edge);
        ...
        int ccount, CCtsp_lpclique *clist, double *cval)
{
    ...
    *bobj = (CCtsp_branchobj *) NULL;
    ...
    double prob_edge = -1.0;

    for (i = 0; i < ecount; i++) {
        insert_sblist (slist, eval[i], i, prob_edge);
    }
    for (i = 0; i < ccount; i++) {
        insert_sblist (slist, cval[i], i + ecount, prob_edge);
    }
        ...
        if (!lp->graph.edges[j].fixed && !lp->graph.edges[j].branch) {
            if (x[i] > 0.5) {
                if (1.0 - x[i] > maxdiff){
                    maxdiff = 1.0 - x[i];
                    besti = i;
                    bestprob = lp->graph.edges[j].prob;
                }
                else if (USE_PVAR && (fabs(1.0 - x[i] - maxdiff) < FRAC_TOL)){
                    nn_edges_probs.total_frac_ties++;
                    if(lp->graph.edges[j].prob > bestprob){
                        nn_edges_probs.frac_ties_count++;
                        bestprob = lp->graph.edges[j].prob;
                        besti = i;
                    }
                }
                ...
                if (x[i] > maxdiff){
                    ...
                    besti = i;
                    bestprob = lp->graph.edges[j].prob;
                } else if (USE_PVAR &&  fabs(x[i] - maxdiff) < FRAC_TOL) {
                    nn_edges_probs.total_frac_ties++;
                    if(lp->graph.edges[j].prob > bestprob) {
                        nn_edges_probs.frac_ties_count++;
                        bestprob = lp->graph.edges[j].prob;
...
double smooth_weight(double p, double w1, double w2, double alpha) {
    // Avoid log(0)
    if (p < 1e-10) p = 1e-10;
    ...
    // Adjust entropy with exponent alpha
    double adjusted = pow(entropy, alpha);

    // Interpolate between w1 and w2
    return w1 - (w1 - w2) * adjusted;
}
        ...
        int **elist, double **eval, int silent)
{
    ...
    *elist = (int *) NULL;
    ...
    for (i = 0; i < ncand; i++) {
        ...
        if(USE_PVAR){
            prob_edge = lp->graph.edges[candlist[i]].prob;
        ...
        insert_sblist (slist, sval, candlist[i], prob_edge);
        meanval += sval;
    }
        ...
        int **list, int silent)
{
    ...
    *list = (int *) NULL;
    ...
    for (i = 0; i < ngood; i++) {
        ...
        if(USE_PVAR){
            prob_edge = lp->graph.edges[goodlist[i]].prob;
            weight = smooth_weight(prob_edge, TSP_BRANCH_STRONG_WEIGHT, TSP_BRANCH_STRONG_FIRST_WEIGHT, 1);
            sval = H_TSP_BRANCH_STRONG_VAL (downpen[i], uppen[i], weight);
        }
        else{
            prob_edge = -1.0;
        ...
        insert_sblist (slist, sval, goodlist[i], prob_edge);
    }
...
static void insert_sblist (sbitem *list, double val, int name, double prob_edge)
{
    insert_sblist4 (list, val, name, -1, -1, -1, prob_edge);
}

static void insert_sblist4 (sbitem *list, double val, int name, int name2,
        int name3, int name4, double prob_edge)
{
    int k;

    if (list[0].val < val) {
        for (k = 0; (list[k+1].val < val || (USE_PVAR && 
                                            fabs(list[k+1].val - val) < FRAC_TOL && 
                                            prob_edge > list[k+1].prob)); k++) {
            
            if (!(list[k+1].val < val)) {
                nn_edges_probs.total_pvar_ties++;
                nn_edges_probs.pvar_ties_count++;
            }
            
            list[k] = list[k+1];
        }

        if (!(list[k+1].val < val) && USE_PVAR && fabs(list[k+1].val - val) < FRAC_TOL){
            nn_edges_probs.total_pvar_ties++;
        }
        ...
        int *ngot, CCtsp_lpclique **bcliques, double **bval, int silent)
{
    ...
    *bcliques = (CCtsp_lpclique *) NULL;
    ...
    for (i = 0; i < ccount; i++) {
        ...
        sval = TSP_BRANCH_STRONG_VAL (down, up);
        insert_sblist (slist, sval, i, prob_edge);
        meanval += sval;
    }
    ...
    for (i = 0; i < lpcount; i++) {
        ...
        sval = TSP_BRANCH_STRONG_VAL (down, up);
        insert_sblist (slist, sval, lpcand[i], prob_edge);
        meanval += sval;
    }
        ...
        for (i = 0, batch = 0; i < ccount; i++) {
            ...
            if ((i+1) % TSP_STRONG_GETWEIGHT_BATCH == 0 || i == ccount - 1) {
                ...
                for (j = 0; j < cr.rowcnt; j++) {
                    ...
                    insert_sblist (slist, sval, batch+j, -1.0);
                }
                CCtsp_free_lprow (&cr);
                batch = i+1;
            }
...
int CCtsp_bb_splitprob (char *probname, int probnum, int ncount,
        ...
        int *prune0, int *prune1, double *prob0, double *prob1, int silent, CCrandstate *rstate)
{
    int rval = 0;
    CCtsp_lp *lp = (CCtsp_lp *) NULL;
    ...
    *prune = 0;
    ...
    if (b->ends[0] != -1) {
        ...
        if (side == 0) {
            ...
            b->rhs = 1;
            prob = lp->graph.edges[CCtsp_find_edge (&lp->graph, b->ends[0], b->ends[1])].prob;
            count = 1;
        }
    } else {
        if (side == 0) {
    ...
    if(USE_PNODE || USE_PVAR){
        double graph_prob = 0.0;
        double count_edge = 0.0;
        int id_res_src, id_res_dest, edge_id;

        for (int j = 0; j < lp->result.ecount; j++){
            id_res_src = lp->result.elist[2*j];
            id_res_dest = lp->result.elist[2*j + 1];
            edge_id = CCtsp_find_edge(&lp->graph, id_res_src, id_res_dest);

            if (edge_id == -1){
                fprintf(stderr, "Edge not found in graph\n"); fflush(stdout);
                rval = 1;
                goto CLEANUP;
            }

            graph_prob = graph_prob + lp->graph.edges[edge_id].prob * lp->result.x[j];
            count_edge = count_edge + lp->result.x[j];
        }

        if (count_edge != 0.0){
            lp->graph.prob = graph_prob / count_edge;
        }
        else{
            fprintf (stderr, "no variable in LP\n");
            rval = 1; goto CLEANUP;
        }
    }
        ...
        int *tour, char *fname, int writeedges, int silent)
{
    ...
    if (dat) {
        for (i = 1; i < ncount; i++) {
            ...
            if (USE_PVAR || USE_PNODE){
                prob += nn_edges_probs.edge_probs[perm[tour[i-1]]][perm[tour[i]]];
            }

        }
        len += (double) CCutil_dat_edgelen (tour[ncount-1], tour[0], dat);
        if (USE_PVAR || USE_PNODE){
            prob += nn_edges_probs.edge_probs[perm[tour[ncount-1]]][perm[tour[0]]];
            printf("\nTOUR PROB: %f\n", prob/ncount);
            printf("\nPNODE TIES: %d of which used: %d\n", nn_edges_probs.total_pnode_ties, 
                   nn_edges_probs.pnode_ties_count);
            printf("\nPVAR TIES: %d of which used: %d\n", nn_edges_probs.total_pvar_ties, 
                   nn_edges_probs.pvar_ties_count);
            printf("\nFRAC TIES: %d of which used: %d\n", nn_edges_probs.total_frac_ties, 
                   nn_edges_probs.frac_ties_count);
        }
        if (!silent) {
